...
def convert_examples_to_features(js,tokenizer,args):
    prefix_tokens = [tokenizer.cls_token,"<encoder-only>",tokenizer.sep_token]
    prefix_ids = tokenizer.convert_tokens_to_ids(prefix_tokens)
    postfix_tokens = [tokenizer.sep_token]
    postfix_ids = tokenizer.convert_tokens_to_ids(postfix_tokens)
    ...
    code_ids += [tokenizer.pad_token_id]*args.nl_length
    code_ids = prefix_ids + code_ids[:args.code_length + args.nl_length - 4] + postfix_ids
    ...
    nl_ids = [tokenizer.pad_token_id]*args.code_length + nl_ids
    nl_ids = prefix_ids + nl_ids[:args.code_length + args.nl_length - 4] + postfix_ids
    
    return InputFeatures(code_ids, nl_ids)
    ...
    def __init__(self, tokenizer, args, file_path):
        ...
        with open(file_path) as f:
            for line in f:
                ...
                self.data.append({'code_tokens':code, 'docstring_tokens':nl})
...
def train(args, model, tokenizer):
    ...
    for idx in range(args.num_train_epochs): 
        ...
        if result < best_result:
            ...
            logger.info("  "+"*"*20)
            logger.info("  Best loss:%s",round(best_result,4))
            logger.info("  "+"*"*20)
            patience += 1
            if patience == 2:
                exit()
...
def evaluate(args, model, tokenizer, file_name, eval_when_training=False):
    query_dataset = MyDataset(tokenizer, args, file_name)
